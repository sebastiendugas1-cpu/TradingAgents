...
Write-Host "=== SLICE 20D ADD PRIVATE CLIENT TRANSPORT INTEGRATION TEST TO SAFETY REGRESSION SUITE ==="
...
if (-not (Test-Path ".\scripts\test_kraken_private_client_transport_integration.py")) {
    throw "Missing Slice 20C private client transport integration test script."
}

$runner = Get-Content $runnerPath -Raw

if ($runner -notlike "*scripts/test_kraken_private_client_transport_integration.py*") {
...
'    "scripts/test_kraken_private_transport_shell.py",
    ...
    if ($runner -notlike "*scripts/test_kraken_private_client_transport_integration.py*") {
        throw "Failed to insert private client transport integration test into DEFAULT_SAFETY_TESTS."
    }

    Set-Content -Path $runnerPath -Value $runner -Encoding UTF8
    Write-Host "[UPDATED] $runnerPath"
} else {
    Write-Host "[SKIPPED] $runnerPath already includes private client transport integration test"
}
...
if ($test -notlike '*test_default_safety_tests_include_private_client_transport_integration_test*') {
    $insert = @'

def test_default_safety_tests_include_private_client_transport_integration_test() -> None:
    assert "scripts/test_kraken_private_client_transport_integration.py" in DEFAULT_SAFETY_TESTS

    print("[OK] default safety tests include private client transport integration test")
'@
...
'    test_default_safety_tests_include_private_transport_shell_test()
    test_default_safety_tests_include_private_client_transport_integration_test()
    test_regression_suite_runs_subset_successfully()'
    )

    Write-Host "[UPDATED] $testPath"
} else {
    Write-Host "[SKIPPED] $testPath already includes private client transport integration assertion"
}
...
Add-DocBlockOnce -Path ".\docs\03_ROADMAP.md" -Marker "Slice 20D — Add Private Client Transport Integration Test to Master Safety Regression Suite" -Block $roadmapBlock
Add-DocBlockOnce -Path ".\docs\10_EXECUTION_AND_RISK_CONTROLS.md" -Marker "Slice 20D — Private Client Transport Integration Test Added to Safety Regression Suite" -Block $controlsBlock
Add-DocBlockOnce -Path ".\docs\11_DECISION_LOG.md" -Marker "Slice 20D Decision — Protect Private Client Transport Integration in Master Regression Suite" -Block $decisionBlock
...
Write-Host ""
Write-Host "Slice 20D script completed."
